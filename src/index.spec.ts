...
function getTextbox(displayElement: HTMLElement) {
  return displayElement.querySelector<HTMLTextAreaElement>(
...
function getVideo(displayElement: HTMLElement) {
  return displayElement.querySelector<HTMLVideoElement>(
    "#jspsych-video-text-response-stimulus"
  );
}
...
describe("plugin-video-text-response", () => {
  ...
  ])("%s: false strips the corresponding character class", async (param, value, input, expected) => {
    const { displayElement } = await startTimeline([
      {
        type: jsPsychVideoTextResponse,
        stimulus: ["video.mp4"],
